...
const DeanReviewForms = () => {
  ...
  const loadForms = async () => {
    ...
    try {
      ...
      // סינון רק טפסים מהפקולטה שלי
      // הערה: צריך להוסיף facultyID ל-API או לחשב לפי departmentID
      const facultyForms = allForms.filter(instance => 
        instance.facultyID === user.facultyID || 
        // גיבוי לחישוב לפי מחלקה אם אין facultyID
        [100, 101, 102, 103, 104].includes(instance.userDepartmentID) // הנדסה = 1
      );

      setForms(facultyForms);
    } catch (err) {
  ...
  const filterForms = () => {
    ...
    switch (activeTab) {
      case 'pending':
        ...
        break;
      case 'approved':
        filtered = filtered.filter(f => f.currentStage === 'ApprovedByDean');
        break;
      case 'rejected':
    ...
    filtered.sort((a, b) => new Date(b.lastModifiedDate) - new Date(a.lastModifiedDate));
    ...
    <Container>
      ...
      <Row className="mb-4">
        <Col>
          ...
          <p className="text-muted">בדוק ואשר טפסים שאושרו על ידי ראשי המחלקות</p>
        </Col>
      </Row>
                ...
                <Tab 
                  ...
                  title={
                    <span>
                      ...
                      אושרו על ידי
                      {forms.filter(f => f.currentStage === 'ApprovedByDean').length > 0 && (
                        <Badge bg="success" className="ms-1">
                          {forms.filter(f => f.currentStage === 'ApprovedByDean').length}
                        </Badge>
                      )}
                    </span>
                  } 
              ...
              {filteredForms.length === 0 ? (
                <div className="text-center py-5">
                  ...
                  <h4 className="mt-3 text-muted">
                    ...
                    {activeTab === 'approved' && 'אין טפסים שאושרו על ידך'}
                    {activeTab === 'rejected' && 'אין טפסים שנדחו על ידך'}
                  </h4>
                </div>
              ) : (
      ...
      <Modal show={showReviewModal} onHide={() => setShowReviewModal(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>
            {reviewAction === 'approve' ? 'אישור טופס בפקולטה' : 'דחיית טופס בפקולטה'}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {selectedInstance && (
            <>
              ...
              <div className="mb-3">
                <strong>מחלקה:</strong> {selectedInstance.userDepartmentID}
              </div>
              <div className="mb-3">
